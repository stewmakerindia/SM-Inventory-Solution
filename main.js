...
const SUCCESS_COLOR = "#16a34a"; // A nice green color for success
// =================================================================
...
function attachFormListeners(tabContent) { /* ... THIS IS THE MAIN CHANGE ... */ }
function initChefView() { /* ... no changes ... */ }
async function main() { /* ... no changes ... */ }
...
generateWhatsAppMessage = function(orderedItems) {
...
renderSuperCategorySelector = function(formType) {
...
renderForm = function(items, formType) {
    ...
    const buttonText = formType === 'order' ? 'SUBMIT ORDER' : 'SAVE TO HQ';
...
renderProductionPlan = function(plan, outlet) {
...
// *** THE ONLY FUNCTION WITH MAJOR CHANGES IS THIS ONE ***
attachFormListeners = function(tabContent) {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            // --- Part 1: Handle WhatsApp button click ---
            if (submitButton.dataset.action === 'send-whatsapp') {
                const message = submitButton.dataset.whatsappMessage;
                window.open(`https://wa.me/?text=${message}`, '_blank');
                // Optional: Reset button after sending
                submitButton.disabled = true;
                submitButton.textContent = 'WhatsApp Opened!';
                return; // Stop here
            }

            // --- Part 2: Handle regular form submission ---
            ...
            if (formType === 'order') {
                // Transform the button into a WhatsApp button
                const message = generateWhatsAppMessage(orderedItemsForWhatsApp);
                submitButton.dataset.whatsappMessage = message;
                submitButton.dataset.action = 'send-whatsapp';
                submitButton.style.backgroundColor = SUCCESS_COLOR;
                submitButton.innerHTML = '✅ Order Saved! Send on WhatsApp';
            } else {
                alert('Inventory saved successfully!');
                // Reset the regular inventory button
                submitButton.innerHTML = 'SAVE TO HQ';
            }
        });
    }
    ...
    tabContent.querySelectorAll('.back-button').forEach(button => {
        button.addEventListener('click', () => {
            ...
            const activeTabButton = tabs.querySelector(`button[style*="${BRAND_COLOR}"], button[style*="${SUCCESS_COLOR}"]`);
