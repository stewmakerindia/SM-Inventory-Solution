...
async function getSheetData() { /* ... no changes ... */ }
async function postSheetData(sheetName, data) { /* ... no changes ... */ }
function getSortedCategories(items, orderList) { /* ... no changes ... */ }
function generateWhatsAppMessage(orderedItems) { /* ... no changes ... */ }

// --- HTML TEMPLATES ---
function renderOutletSelector() { /* ... no changes ... */ }
function renderSuperCategorySelector(formType) { /* ... no changes ... */ }
function renderForm(items, formType) { /* ... no changes ... */ }
function renderProductionPlan(plan, outlet) { /* ... no changes ... */ }
function renderSalesDashboard() { /* ... no changes ... */ }

// *** THIS IS THE ONLY FUNCTION THAT HAS CHANGED ***
function renderSalesResults(data) {
    const formatCurrency = (amount) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(amount);
    
    // Helper function to get the 3-letter day of the week from a 'dd/mm/yyyy' string
    ...
    const weekLabel = `(${getDayOfWeek(data.thisWeek.start)}-${getDayOfWeek(data.thisWeek.end)})`;
    ...
    return `
        ...
        <div class="p-4 bg-white rounded-lg shadow-sm border-l-4" style="border-color: ${BRAND_COLOR};">
            <p class="text-sm font-bold text-gray-600">THIS WEEK'S SALES ${weekLabel}</p>
            <div class="flex justify-between items-baseline mt-1">
...
renderSalesDashboard = function() { const today = new Date().toISOString().split('T')[0]; return `<h2 class="text-xl font-bold mb-4">Sales Information</h2><div class="mb-6"><label for="sales-date-picker" class="block text-sm font-medium text-gray-700 mb-1">Select a Date to View Report</label><input type="date" id="sales-date-picker" value="${today}" class="w-full p-2 border border-gray-300 rounded-lg"></div><div id="sales-results" class="space-y-4"><div class="text-center p-4">Loading sales data...</div></div>`; };
attachFormListeners = function(tabContent) { const form = tabContent.querySelector('form'); if (form) { form.addEventListener('submit', async (e) => { e.preventDefault(); const submitButton = form.querySelector('button[type="submit"]'); if (submitButton.dataset.action === 'send-whatsapp') { const message = submitButton.dataset.whatsappMessage; window.open(`https://wa.me/?text=${message}`, '_blank'); submitButton.disabled = true; submitButton.textContent = 'WhatsApp Opened!'; return; } const formType = form.id.split('-')[0]; const logSheet = formType === 'order' ? 'Order_Log' : 'Inventory_Log'; const dataToSave = [], orderedItemsForWhatsApp = []; const timestamp = new Date().toISOString(); for (let input of form.querySelectorAll('input[type="number"]')) { if (input.value && input.value.trim() !== '' && parseFloat(input.value) > 0) { const quantity = parseFloat(input.value); dataToSave.push([timestamp, currentOutlet, input.dataset.item, quantity]); if (formType === 'order') { orderedItemsForWhatsApp.push({ name: input.dataset.item, quantity: quantity, unit: input.dataset.unit, superCategory: input.dataset.supercategory }); } } } if (dataToSave.length === 0) { alert('Please enter a value for at least one item.'); return; } submitButton.disabled = true; submitButton.innerHTML = 'Saving...'; await postSheetData(logSheet, dataToSave); submitButton.disabled = false; form.reset(); if (formType === 'order') { const message = generateWhatsAppMessage(orderedItemsForWhatsApp); submitButton.dataset.whatsappMessage = message; submitButton.dataset.action = 'send-whatsapp'; submitButton.style.backgroundColor = SUCCESS_COLOR; submitButton.innerHTML = '✅ Order Saved! Send on WhatsApp'; } else { alert('Inventory saved successfully!'); submitButton.innerHTML = 'SAVE TO HQ'; } }); } tabContent.querySelectorAll('.back-button').forEach(button => { button.addEventListener('click', () => { const tabs = document.getElementById('chef-tabs'); const activeTabButton = tabs.querySelector(`button[style*="${BRAND_COLOR}"], button[style*="${SUCCESS_COLOR}"]`); if (activeTabButton) { activeTabButton.click(); } }); }); };
